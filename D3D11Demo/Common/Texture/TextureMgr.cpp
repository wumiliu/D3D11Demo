...
{
	return GetTexture((BaseFunction::UnicodeToANSI(filename.c_str())));
}
...
{
	return CreateTexture((BaseFunction::UnicodeToANSI(filename.c_str())));
}
...
ID3D11ShaderResourceView* TextureMgr::CreateTextureDDs(std::string filename)
{
	...
	else
	{
		...
		std::wstring szFileName = BaseFunction::gAnsiToUnicode(filename.c_str());
